...
contract LiquidityPool is ERC20Token {
    ...
    function swap(uint256 amountIn, address tokenIn, address tokenOut) public returns (bool) {
         require(tokenIn == addressTokenA || tokenIn == addressTokenB, "Invalid token address");
         require(tokenOut == addressTokenA || tokenOut == addressTokenB, "Invalid token address");
    
         ERC20Token tokenInContract = ERC20Token(tokenIn);
         ERC20Token tokenOutContract = ERC20Token(tokenOut);

         uint256 reserveIn = tokenIn == addressTokenA ? reserveTokenA : reserveTokenB;
         uint256 reserveOut = tokenOut == addressTokenA ? reserveTokenA : reserveTokenB;

         uint256 amountOut = getAmountOut(amountIn, reserveIn, reserveOut);
         require(amountOut > 0, "Insufficient output amount");

        // Apply 0.5% fee
         uint256 feeAmount = amountIn / 200;
         uint256 amountOutWithFee = amountOut - feeAmount;

         require(tokenInContract.transferFrom(msg.sender, address(this), amountIn), "Transfer of token failed");
         require(tokenOutContract.transfer(msg.sender, amountOutWithFee), "Transfer of token failed");
     
         if (tokenIn == addressTokenA) {
             reserveTokenA += amountIn;
             reserveTokenB -= amountOut;
             reserveTokenA -= feeAmount;

         } else {
             reserveTokenB += amountIn;
             reserveTokenA -= amountOut;
            reserveTokenB -= feeAmount;
         }

         emit Sync(reserveTokenA, reserveTokenB);
         return true;
    }

    function removeLiquidity(uint256 liquidityShares) public returns (uint256, uint256) {    
        uint256 amountTokenA = (liquidityShares * reserveTokenA) / totalSupply;
        uint256 amountTokenB = (liquidityShares * reserveTokenB) / totalSupply;

        require(amountTokenA > 0 && amountTokenB > 0, "Insufficient amount");

        require(ERC20Token(addressTokenA).transfer(msg.sender, amountTokenA), "Transfer of token failed");
        require(ERC20Token(addressTokenB).transfer(msg.sender, amountTokenB), "Transfer of token failed");

        reserveTokenA -= amountTokenA;
        reserveTokenB -= amountTokenB;
        totalSupply -= liquidityShares;
